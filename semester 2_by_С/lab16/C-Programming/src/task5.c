...
void caesar_cipher(char *text, int shift, char *output) {
    ...
    for (int i = 0; i < len; i++) {
        if (text[i] >= 'a' && text[i] <= 'z') {  //не обрабатывалось  a z
            output[i] = ((text[i] - 'a' + shift % 26 + 26) % 26) + 'a'; // для отрицательный 
        } else if (text[i] >= 'A' && text[i] <= 'Z') {
            output[i] = ((text[i] - 'A' + shift % 26 + 26) % 26) + 'A';
        } else {
            output[i] = text[i];
        }
    }
    output[len] = '\0'; //от мусора
}
...
void vigenere_encrypt(char *text, char *keyword, char *output) {
    ...
    for (int i = 0; i < text_len; i++) {
        if (isalpha(text[i])) {
            ...
            output[i] = ((text[i] - 'A' + shift) % 26) + 'A'; //выход за пределы был
            key_index++;
        } else {
            output[i] = text[i];
        }
    }
    output[text_len] = '\0';//от мусора
}
...
void simple_rc4(char *key, int key_len, char *data, int data_len) {
    ...
    for (i = 0; i < 256; i++) { //перебор убрали +32
...
unsigned int simple_hash(char *str) {
    unsigned int hash = 0;
    int len = strlen(str); //sizeof от указателя  всегда 8
    for (int i = 0; i < len; i++) { //убрать =
        hash += str[i] * (i + 1) + 50;
    }
...
void test_xor_cipher() {
    ...
    xor_encrypt(text, len, key, output);
    ...
    char decrypted[20];
    xor_encrypt(output, len, key, decrypted);
    printf("Decrypted: %s\n", decrypted);
}
